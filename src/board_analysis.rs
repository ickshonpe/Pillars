...
pub fn scan_for_matches(board: &Board, min_gem_line_length: usize) -> HashSet<P2> {
    ...
    let lines: Vec<Vec<P2>> = get_all_lines(board)
        .into_iter()
        .filter(|line| line.len() >= min_gem_line_length)
        .collect();
    ...
    while x < jewels.len() {
        if let Some(current_jewel) = jewels[x] {
            ...
            while x < jewels.len() {
                ...
                if x == jewels.len() - 1 && matching.len() >= match_length {
                    for i in &matching {
                        matches.insert(*i);
                    }
                }
...
pub fn check_for_collision(board: &Board, column: &Column) -> bool {
    ...
    if board[p.trans_up()] != None {
        return true;
    }
    if board[p.trans_up().trans_up()] != None {
        return true;
    }
    false
}
